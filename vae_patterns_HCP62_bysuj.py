...
mat = scipy.io.loadmat('patterns_to_python.mat')
patterns=mat['patters_63']


my_data = np.zeros((62,1081*989))
for jj in range(989):
    my_data[:,jj*1081:(jj+1)*1081] = np.squeeze(patterns[:,:,jj])
...
vae.load_weights('vae_FCsHCP_dim9.h5')


# err de corr y reconstr
FCerror =[]
NSUB = patterns.shape[2]
FCemp2 = np.zeros((NSUB, original_dim, original_dim))
for i in range(NSUB):
    print(i)
    s1 = my_data[i*1081:(i+1)*1081,:]+1
    s1 = scipy.stats.zscore(s1) # ZSCORE 
    label_s1= np.ones(1081)*1
    s1_data = (s1,label_s1)
...
TIME_POINTS = 1081   # Number of time points that we wanna end up with!
...
for i in range(NSUB):
    ...
    for kk in range(9):
        s1 = my_data[9*i*1081+1081*kk:(9*i+1+kk)*1081,:]
        label_s1= np.ones(1081)*1
        s1_data = (s1,label_s1)
...
with open('eff_conn_rest_dim9.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([structural_out,cc_mean,ssim_mean], f)
